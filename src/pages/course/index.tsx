...
const Course: FC<ICourseProps> = (props) => {
  const { kc, ks, ksbfb } = props;
  return (
    <div className="course_container">
      <div className="star_group_1"></div>
      <div className="star_group_2"></div>
      <div className="map"></div>
      <svg className="path_svg" version="1.1" viewBox="0 0 500 500">
        <path
          className="path"
      ...
      <div className="course_text">
        <span>
          本学期有<span className="stress_text">{kc}</span>门课程
        </span>
        <br />
        <span>
          课时数达到了<span className="stress_text">{ks}</span>小时
        </span>
        <br />
        <span>
          超过了<span className="stress_text">{(ksbfb * 100).toFixed(2)}%</span>的福大学子
        </span>
        <br />
        <br />
        <span>
          用这个时间你可以步行绕福大<span className="stress_text">{ks}</span>圈
        </span>
      </div>
    </div>
  );
